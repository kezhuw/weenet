...
static size_t
_read(int fd, byte_t *buf, size_t len, bool full, int *err) {
	*err = 0;
	size_t n = 0;
again:
	do {
...
static size_t
_write(int fd, byte_t *data, size_t size, bool full, int *err) {
	*err = 0;
	size_t n = 0;
again:
	do {
...
_socket_read(lua_State *L) {
	...
	if (required == 0) {
		byte_t buf[1024];
		int err;
		size_t n = _read(fd, buf, sizeof buf, false, &err);
		if (n == 0) {
	...
	size_t n = (size_t)ctx;
	int err;
	n += _read(fd, buf+n, required-n, true, &err);
	if (n == required) {
...
_socket_write(lua_State *L) {
	...
	int err = 0;
	size_t n = _write(fd, data, size, true, &err);
	if (n == size) {
		lua_pushboolean(L, 1);
		return 1;
	}
...
_socket_connect(lua_State *L) {
	...
	lua_pushfstring(L, "error when connecting to %s:", addr);
